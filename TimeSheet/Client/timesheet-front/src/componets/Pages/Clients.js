...
const Client = ({ color, text, onClick }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setNumberOfPages(Math.ceil(responseClients.totalItems / responseClients.pageSize));
  ...
  return (
    <div>
      <Search setQuery = {setStringQuery}  title = {"Create Client"} createForm ={<CreateClientForm countries = {countries}/>}></Search>
